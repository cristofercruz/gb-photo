...
/* Only the first seven bytes here are spare. $AFF2..$AFFF is the sensor calibration
   block -- twelve threshold references and two check bytes -- in SRAM bank 2, echoed at
   $BFF2 in bank 8. It survives an image slot being erased, and it is measured per
   camera, so anything writing across this range destroys data that can only be
   regenerated by the factory procedure. The array stops short of it deliberately. */
static uint8_t AT(0xAFEB) image_first_unused[7];
...
// static uint8_t AT(0xBFEB) image_second_unused[7];   // $BFF2 onward is the calibration echo
